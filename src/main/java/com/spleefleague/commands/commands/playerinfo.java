...
public class playerinfo extends BasicCommand {
    ...
    public playerinfo(CorePlugin plugin, String name, String usage) {
        super(plugin, new playerinfoDispatcher(), name, usage, Rank.MODERATOR);
    }

    @Endpoint(target = {PLAYER})
    public void playerinfoSelf(SLPlayer slp) {
        playerinfoOther(slp, slp);
    }
    
    @Endpoint(target = {PLAYER})
    public void playerinfoOther(SLPlayer p, @SLPlayerArg(offline = true) SLPlayer target) {
                ...
                new ConnectionResponseHandler("sessions", request, 40) {
                    ...
                    protected void response(JSONObject jsonObject) {
                        Bukkit.getScheduler().runTaskAsynchronously(SpleefLeague.getInstance(), () -> {
                            ...
                            p.sendMessage(ChatColor.DARK_GRAY + "[========== " + ChatColor.GRAY + target.getName() + "'s data " + ChatColor.DARK_GRAY + "==========]");
